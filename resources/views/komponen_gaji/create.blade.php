...
<div class="space-y-6 max-w-5xl mx-auto">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Tambah Komponen Gaji</h2>
                <p class="text-xs text-slate-500 dark:text-slate-400">Tambahkan komponen penghasilan atau potongan baru</p>
            </div>
        </div>
        ...
        <form action="{{ route('komponen-gaji.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="nilai" value="{{ old('nilai') }}" min="0" required placeholder="0" class="bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white text-sm rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5 transition-all">
                </div>
                ...
                <div class="md:col-span-2 lg:col-span-3">
                    ...
                    <textarea name="keterangan" rows="3" placeholder="Deskripsi atau rincian komponen..." class="bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white text-sm rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5 transition-all">{{ old('keterangan') }}</textarea>
                </div>
            </div>
            ...
            <div class="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end gap-3">
                ...
                <button type="submit" class="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold shadow-md shadow-indigo-600/20 transition-all">
                    <i class="bi bi-check2-circle text-base"></i> Simpan Data
                </button>
            </div>
        </form>
    </div>
